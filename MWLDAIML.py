...
def lt_train_test(X_train, X_test, Y_train, Y_true,  L, Sw, Sb, G,  d, l, device, miu_max, miu1_max, miu2_max, miu3_max,
                  alpha, beta, lamda1, lamda2, lamda3, rou, rou1, rou2, rou3, yuzhi,
                  miu, miu1, miu2, miu3):
    W = _one((d, l), device) * 1
    K = _one((l, l), device) * 1
    Z = _one((l, l), device) * 1
    ...
    while step <= 350:
        ...
        W = torch.from_numpy(scipy.linalg.solve_sylvester(
            (torch.mm(X_train.T, X_train) + 2 * alpha * X_train.T @ L @ X_train + 2 * lamda1 * torch.eye(d, dtype=torch.float64).cuda()).cpu().numpy(),
            (miu1 * torch.mm(K, K.T) + miu2 * torch.mm(H, H.T)).cpu().numpy(),
            (X_train.T @ Y_train @ G + miu1 * torch.mm(M, K.T) + miu2 * torch.mm(N, H.T) - torch.mm(Lambda1, K.T) - torch.mm(
    ...
    Y_score = (X_test @ W @ torch.inverse(G)).cpu().numpy()
    Y_pred = np.where(Y_score > yuzhi, 1, 0)
    scorce = mll_metrics(Y_true, Y_pred, Y_score)
    return scorce

def getParameters(X, Y, n, d, l, k, delta, ksai, epsilo_1, epsilo_0, is_cost, device):
    u_1 = torch.mm(Y.T, X) / Y.sum(axis=0).reshape(-1, 1).clamp(min=1e-10)  # 防止除以零
    u_1[Y.sum(axis=0) == 0] = 0
    u_0 = torch.mm((1 - Y).T, X) / (1 - Y).sum(axis=0).reshape(-1, 1).clamp(
        min=1e-10)
    u_0[(1 - Y).sum(axis=0) == 0] = 0

    D_1 = torch.cdist(X, u_1, p=2) ** 2
    mask_1 = (Y == 1)
    ...
    miu_1 = (P_1.T @ X) / P_1.sum(dim=0, keepdim=True).T
    miu_1.nan_to_num_(0)
    miu_0 = (P_0.T @ X) / P_0.sum(dim=0, keepdim=True).T
    miu_0.nan_to_num_(0)

    Miu = ((P_1.T @ X).sum(dim=0) + (P_0.T @ X).sum(dim=0)) / torch.sum(P_1 + P_0)
    Miu.nan_to_num_(0)

    Sw = torch.zeros((d, d), dtype=torch.float64, device=device)  # 初始化 Sw 为零矩阵
    for j in range(l):
        diff_1 = X - miu_1[j]
        diff_0 = X - miu_0[j]
        Sw += 0.5 * ((P_1[:, j:j + 1] * diff_1).T @ diff_1 + (P_0[:, j:j + 1] * diff_0).T @ diff_0)  # 形状 (d, d)
...
def MWLDAIML(X, Y, X_test, Y_true, is_cost, miu_max, miu1_max, miu2_max, miu3_max,
            alpha, beta, lamda1, lamda2, lamda3, rou, rou1, rou2, rou3, miu, miu1, miu2, miu3, yuzhi):
    ...
    (n, d) = X.shape
    l = Y.shape[1]
    Sw, Sb, L, G = getParameters(X, Y, n, d, l, k, delta, ksai, epsilo_1, epsilo_0, is_cost, device)
    scorce = lt_train_test(X, X_test, Y, Y_true, L, Sw, Sb, G, d, l, device, miu_max, miu1_max, miu2_max, miu3_max,
                  alpha, beta, lamda1, lamda2, lamda3, rou, rou1, rou2, rou3, yuzhi,
                  miu, miu1, miu2, miu3)
